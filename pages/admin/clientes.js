...
export default function AdminClientes() {
  ...
  return (
    ...
    >
      ...
      {/* Tarjetas KPI */}
      <div className="mb-6 grid grid-cols-2 gap-3 sm:grid-cols-2 lg:grid-cols-4 sm:gap-4">
        <div className="rounded-3xl border border-slate-200/80 bg-white/90 p-4 sm:p-5 shadow-sm backdrop-blur">
          <div className="flex items-center justify-between">
            <span className="text-[10px] sm:text-xs font-bold uppercase tracking-wider text-slate-500">Total Clientes</span>
            <AdminMetricIcon variant="default">
              <UsersIcon className="h-4 w-4 sm:h-5 sm:w-5 text-pink-600" />
            </AdminMetricIcon>
          </div>
          <p className="mt-2 text-2xl sm:text-3xl font-extrabold text-slate-800">{stats.total}</p>
          <p className="mt-0.5 text-[10px] sm:text-xs text-slate-500">Clientes registrados</p>
        </div>

        <div className="rounded-3xl border border-slate-200/80 bg-white/90 p-4 sm:p-5 shadow-sm backdrop-blur">
          <div className="flex items-center justify-between">
            <span className="text-[10px] sm:text-xs font-bold uppercase tracking-wider text-slate-500">Frecuentes</span>
            <AdminMetricIcon variant="success">
              <SparkleIcon className="h-4 w-4 sm:h-5 sm:w-5 text-emerald-600" />
            </AdminMetricIcon>
          </div>
          <p className="mt-2 text-2xl sm:text-3xl font-extrabold text-emerald-600">{stats.frequent}</p>
          <p className="mt-0.5 text-[10px] sm:text-xs text-slate-500">3 o más citas</p>
        </div>

        <div className="rounded-3xl border border-slate-200/80 bg-white/90 p-4 sm:p-5 shadow-sm backdrop-blur">
          <div className="flex items-center justify-between">
            <span className="text-[10px] sm:text-xs font-bold uppercase tracking-wider text-slate-500">Fidelidad</span>
            <AdminMetricIcon variant="warning">
              <GiftIcon className="h-4 w-4 sm:h-5 sm:w-5 text-amber-500" />
            </AdminMetricIcon>
          </div>
          <p className="mt-2 text-2xl sm:text-3xl font-extrabold text-amber-600">{stats.rewardsAvailable}</p>
          <p className="mt-0.5 text-[10px] sm:text-xs text-slate-500">Premio disponible</p>
        </div>

        <div className="rounded-3xl border border-slate-200/80 bg-white/90 p-4 sm:p-5 shadow-sm backdrop-blur">
          <div className="flex items-center justify-between">
            <span className="text-[10px] sm:text-xs font-bold uppercase tracking-wider text-slate-500">Prom. Citas</span>
            <AdminMetricIcon variant="default">
              <ChartIcon className="h-4 w-4 sm:h-5 sm:w-5 text-slate-700" />
            </AdminMetricIcon>
          </div>
          <p className="mt-2 text-2xl sm:text-3xl font-extrabold text-slate-800">{stats.avgBookings}</p>
          <p className="mt-0.5 text-[10px] sm:text-xs text-slate-500">Por cliente</p>
        </div>
      </div>

      {/* Barra Superior de Acciones y Filtros */}
      <div className="mb-6 flex flex-col gap-3 rounded-3xl border border-slate-200/80 bg-white/90 p-4 sm:p-5 shadow-sm backdrop-blur lg:flex-row lg:items-center lg:justify-between">
        <div className="relative flex-1">
          <input
            ...
            className="w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 pl-11 text-base sm:text-sm font-medium text-slate-800 transition focus:border-pink-500 focus:bg-white focus:outline-none focus:ring-2 focus:ring-pink-500/20"
          />
        ...
        <div className="flex flex-wrap items-center gap-2">
          {[
            ...
            <button
              ...
              className={`rounded-2xl px-3.5 py-2 sm:px-4 sm:py-2.5 text-xs font-bold transition-all ${
                filterMode === tab.id
          ...
          <button
            ...
            className="inline-flex items-center gap-2 rounded-2xl border border-slate-200 bg-slate-900 px-3.5 py-2 sm:px-4 sm:py-2.5 text-xs font-bold text-white shadow transition hover:bg-slate-800"
          >
      ...
      {/* Barra de Acciones Flotante al Seleccionar Clientes en Móvil y Desktop */}
      {selectedEmails.size > 0 && (
        <div className="sticky top-16 sm:top-20 z-30 mb-6 flex flex-col sm:flex-row items-center justify-between rounded-2xl bg-gradient-to-r from-pink-600 to-rose-600 p-4 text-white shadow-xl gap-3">
          <div className="flex items-center gap-3 w-full sm:w-auto justify-between sm:justify-start">
            <span className="flex h-8 w-8 items-center justify-center rounded-full bg-white/20 text-sm font-black">
              {selectedEmails.size}
            </span>
            <p className="text-xs sm:text-sm font-bold">Clientes seleccionados</p>
          </div>
          <div className="flex items-center gap-2 w-full sm:w-auto">
            <button
              onClick={handleOpenEmailModal}
              className="flex-1 sm:flex-initial inline-flex items-center justify-center gap-2 rounded-xl bg-white px-4 py-2.5 text-xs font-extrabold text-pink-700 shadow hover:bg-pink-50 transition min-h-[44px]"
            >
            ...
            <button
              ...
              className="rounded-xl bg-pink-800/60 px-3 py-2.5 text-xs font-bold hover:bg-pink-800 transition min-h-[44px]"
            >
              Desmarcar
            </button>
          </div>
      ...
      {/* Lista / Tabla de Clientes (Híbrida: Tarjetas en Móvil, Tabla en Desktop) */}
      {loading ? (
        <div className="flex flex-col items-center justify-center rounded-3xl border border-slate-200 bg-white p-12 text-center shadow-sm">
          ...
          <p className="mt-4 text-sm font-semibold text-slate-600">Cargando clientes...</p>
        </div>
      ) : error ? (
              ...
              return (
                <div
                  ...
                  className={`rounded-3xl border p-4 transition shadow-sm ${
                    isSelected ? 'border-pink-500 bg-pink-50/90 ring-2 ring-pink-500/20' : 'border-slate-200/80 bg-white'
                  }`}
                >
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex items-center gap-3">
                      ...
                      />
                      <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-full bg-gradient-to-tr from-pink-500 to-rose-400 font-bold text-white shadow-sm text-base">
                        {initial}
                      </div>
                      <div>
                        <p className="font-bold text-slate-900 text-base leading-tight">{client.name}</p>
                        <p className="text-xs text-slate-500">{client.email}</p>
                      </div>
                    </div>

                    {client.loyalty?.rewardAvailable ? (
                      <span className="shrink-0 rounded-full bg-amber-100 p-1.5 text-amber-800" title="Premio Disponible">
                  ...
                  <div className="mt-3 flex items-center justify-between border-t border-slate-100 pt-3 text-xs text-slate-600">
                    <div>
                      <span className="font-bold text-slate-800">{client.totalReservations} citas</span> ({client.attendedCount} asistidas)
                    </div>
                  ...
                  {/* Acciones Móviles con Tap Targets Grandes (44px) */}
                  <div className="mt-4 flex items-center gap-2 pt-1">
                    <button
                      onClick={() => handleOpenQuickBooking(client)}
                      className="flex-1 inline-flex items-center justify-center gap-1.5 rounded-2xl bg-pink-600 px-4 py-3 text-xs font-bold text-white shadow-sm transition hover:bg-pink-700 min-h-[44px]"
                    >
                    ...
                    {waUrl && (
                      <a
                        ...
                        className="inline-flex h-11 w-11 items-center justify-center rounded-2xl bg-emerald-500 text-white shadow transition hover:bg-emerald-600 min-h-[44px] min-w-[44px]"
                    ...
                    <button
                      ...
                      className="inline-flex items-center justify-center rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-xs font-bold text-slate-700 shadow-sm transition hover:bg-slate-100 min-h-[44px]"
                    >
                      Detalle
                    </button>
                  </div>
      ...
      {/* --- MODAL 1: AGENDAMIENTO RÁPIDO (BOTTOM SHEET EN MÓVIL, CENTRADO EN DESKTOP) --- */}
      {quickBookingClient && (
        <ModalPortal>
          <div className="fixed inset-0 z-[9999] flex items-end sm:items-center justify-center bg-slate-900/60 p-0 sm:p-4 backdrop-blur-sm overflow-y-auto">
            <div className="w-full max-w-lg rounded-t-3xl sm:rounded-3xl bg-white p-6 shadow-2xl my-0 sm:my-auto max-h-[90vh] overflow-y-auto">
              ...
              <form onSubmit={handleConfirmQuickBooking} className="mt-4 space-y-4 text-sm">
                <div>
                  ...
                  <select
                    ...
                    className="w-full rounded-2xl border border-slate-200 bg-slate-50 p-3.5 text-base sm:text-sm text-slate-800 font-semibold focus:border-pink-500 focus:outline-none"
                  >
                    {services.map((svc) => (
                    ...
                    <div className="grid grid-cols-3 gap-2">
                      {availableSlots.map((slot) => (
                        <button
                          ...
                          className={`rounded-xl py-3 sm:py-2 text-xs font-bold transition min-h-[44px] ${
                ...
                <div className="flex justify-end gap-2 border-t border-slate-100 pt-4">
                  <button
                    ...
                    className="rounded-2xl border border-slate-200 bg-slate-100 px-5 py-3 text-xs font-bold text-slate-700 min-h-[44px]"
                  >
                    Cancelar
                  </button>
                  <button
                    ...
                    className="rounded-2xl bg-pink-600 px-6 py-3 text-xs font-bold text-white shadow hover:bg-pink-700 disabled:opacity-50 min-h-[44px]"
                  >
      ...
      {/* --- MODAL 2: EDITOR DE CAMPAÑA / EMAIL MASIVO (BOTTOM SHEET EN MÓVIL) --- */}
      {showEmailModal && (
        <ModalPortal>
          <div className="fixed inset-0 z-[9999] flex items-end sm:items-center justify-center bg-slate-900/60 p-0 sm:p-4 backdrop-blur-sm overflow-y-auto">
            <div className="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-t-3xl sm:rounded-3xl bg-white p-6 shadow-2xl my-0 sm:my-auto">
              <div className="w-12 h-1.5 bg-slate-300 rounded-full mx-auto mb-3 sm:hidden" />
              <div className="flex items-center justify-between border-b border-slate-100 pb-4">
                <div className="flex items-center gap-2">
              ...
              <form onSubmit={handleSendOrScheduleEmail} className="mt-4 space-y-4 text-sm">
                ...
                <div>
                  ...
                  <textarea
                    ...
                    className="w-full rounded-2xl border border-slate-200 bg-slate-50 p-3.5 text-base sm:text-sm text-slate-800 font-medium focus:border-pink-500 focus:outline-none"
                    required
                  />
                </div>
                ...
                <div className="rounded-2xl bg-slate-50 p-4">
                  <div className="flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-6">
                    <label className="flex items-center gap-2 text-xs font-bold text-slate-700 cursor-pointer min-h-[44px]">
                      <input
                        type="radio"
                        name="emailMode"
                  ...
                  {emailForm.isScheduled && (
                    <div className="mt-3">
                      ...
                      <input
                        ...
                        className="w-full rounded-xl border border-slate-300 p-3 text-base sm:text-xs font-bold"
                      />
                    </div>
                  )}
                </div>
      ...
      {/* --- MODAL 3: HISTORIAL & SUSPENSIÓN DE CAMPAÑAS (BOTTOM SHEET EN MÓVIL) --- */}
      {showCampaignsModal && (
        <ModalPortal>
          <div className="fixed inset-0 z-[9999] flex items-end sm:items-center justify-center bg-slate-900/60 p-0 sm:p-4 backdrop-blur-sm overflow-y-auto">
            <div className="max-h-[90vh] w-full max-w-3xl overflow-y-auto rounded-t-3xl sm:rounded-3xl bg-white p-6 shadow-2xl my-0 sm:my-auto">
              <div className="w-12 h-1.5 bg-slate-300 rounded-full mx-auto mb-3 sm:hidden" />
              <div className="flex items-center justify-between border-b border-slate-100 pb-4">
                <div className="flex items-center gap-2">
                      ...
                      <div key={camp.id} className="flex flex-col sm:flex-row sm:items-center justify-between py-3.5 gap-2 text-xs">
                        ...
                        {camp.status === 'PENDIENTE' && (
                          <button
                            ...
                            className="inline-flex items-center justify-center gap-1 rounded-xl border border-rose-200 bg-rose-50 px-4 py-2.5 font-bold text-rose-700 hover:bg-rose-100 transition min-h-[44px]"
                          >
              ...
              <div className="mt-6 flex justify-end border-t border-slate-100 pt-4">
                <button
                  ...
                  className="rounded-2xl border border-slate-200 bg-slate-100 px-6 py-3 text-xs font-bold text-slate-700 min-h-[44px]"
                >
                  Cerrar
                </button>
              </div>
      ...
      {/* MODAL 4: DETALLE CLIENTE (BOTTOM SHEET EN MÓVIL) */}
      {selectedClient && (
        <ModalPortal>
          <div className="fixed inset-0 z-[9999] flex items-end sm:items-center justify-center bg-slate-900/60 p-0 sm:p-4 backdrop-blur-sm overflow-y-auto">
            <div className="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-t-3xl sm:rounded-3xl bg-white p-6 shadow-2xl my-0 sm:my-auto">
              ...
              <div className="my-6 grid grid-cols-2 gap-3 sm:grid-cols-3">
                <div className="rounded-2xl bg-slate-50 p-4 text-center">
                  <span className="text-[10px] sm:text-xs font-bold text-slate-400 uppercase">Total Citas</span>
                  <p className="mt-1 text-2xl font-black text-slate-800">{selectedClient.totalReservations}</p>
                </div>
                <div className="rounded-2xl bg-emerald-50 p-4 text-center">
                  <span className="text-[10px] sm:text-xs font-bold text-emerald-600 uppercase">Asistidas</span>
                ...
                <div className="col-span-2 sm:col-span-1 rounded-2xl bg-pink-50 p-4 text-center">
                  <span className="text-[10px] sm:text-xs font-bold text-pink-600 uppercase">Fidelidad</span>
              ...
              <div className="mt-6 flex flex-col sm:flex-row justify-end gap-2.5 border-t border-slate-100 pt-4">
                <button
                  onClick={() => {
                  ...
                  className="inline-flex items-center justify-center gap-1.5 rounded-2xl bg-pink-600 px-5 py-3 text-xs font-bold text-white shadow hover:bg-pink-700 transition min-h-[44px]"
                >
                  <UserPlusIcon className="h-4 w-4 text-white" />
                  Agendar Cita Rápida
                </button>

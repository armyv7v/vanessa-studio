...
import { fetchAvailability, postBooking } from '../lib/api';

// ==== CONFIG DE CALENDARIO (UI) ====
// Semanas visibles
const WEEKS_VISIBLE = 3; // 3 semanas
// Mostrar sábados y domingos por defecto:
const ENABLE_SATURDAY = true;
const ENABLE_SUNDAY_DEFAULT = true; // true = mostrar; false = ocultar todos los domingos
// Desactivar domingos específicos por mes: { "YYYY-MM": [1,3] } => domingo 1 y 3 del mes
...
function getSundayIndexInMonth(date) {
  // 1..5 para el domingo N del mes
...
function isSundayDisabled(date) {
  if (!ENABLE_SUNDAY_DEFAULT) return true;
  const list = DISABLE_SUNDAYS_BY_MONTH[ymKey(date)];
  if (!list?.length) return false;
  const idx = getSundayIndexInMonth(date);
...
export default function Home() {
  ...
  // Días visibles (4 semanas)
  const nextDays = useMemo(() => {
    const days = [];
    const today = startOfDay(new Date());
    const total = WEEKS_VISIBLE * 7;
    for (let i = 0; i < total; i++) {
      const d = addDays(today, i);
      const dow = d.getDay(); // 0 dom, 6 sab
      if (dow === 6 && !ENABLE_SATURDAY) continue;
      if (dow === 0 && isSundayDisabled(d)) continue;
      days.push(d);
    }
    return days;
  }, []);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        // Si es hoy: filtrar horas pasadas (front)
        const todayStr = format(new Date(), 'yyyy-MM-dd');
        let filtered = availableSlots;
        if (dateStr === todayStr) {
          const now = new Date();
          const nowHHMM = now.getHours() * 60 + now.getMinutes();
          filtered = availableSlots.filter((hhmm) => {
            const [h, m] = hhmm.split(':').map(Number);
            return (h * 60 + m) >= (nowHHMM + 1); // al menos 1 minuto a futuro
          });
        }
          ...
          <div className="max-w-4xl mx-auto">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-3">
              {nextDays.map((day, index) => {
                ...
                const sel = selectedDate && format(selectedDate, 'yyyy-MM-dd') === format(day, 'yyyy-MM-dd');

...
### Requirement 2: Configuración de horarios no laborables (fetch al montar)
Al montar `BookingFlow`, se obtiene la configuración de días/horas bloqueadas antes de mostrar el calendario.

- **Scenario 2.1:** WHEN se monta el componente THEN se hace `GET /api/gs-check?action=getConfig` (o el `NEXT_PUBLIC_BACKEND_HORARIOS_URL` en dominios `.pages.com`) para obtener días deshabilitados, ordinales (ej. `SUN1`, `SAT3`), fechas y rangos blackout.
- **Scenario 2.2:** WHEN la carga falla THEN el flujo continúa con reglas por defecto (`lib/calendarConfig.js`) sin bloquear la reserva.
...
- **Scenario 6.1:** WHEN se accede a `/extra-cupos` THEN `BookingFlow` recibe `isExtra: true`, ventana 18:00–20:00 y horizonte de 35 días.
- **Scenario 6.2:** La UI muestra un aviso visible del recargo de **$5.000** antes de confirmar.

...
function buildAvailableRange(startDate, endDate, busySlots, serviceDuration = SERVICE_DURATION) {
...
function shouldUseHostedBackend() {
  ...
  return Boolean(
    ...
    || process.env.NEXT_PUBLIC_BACKEND_HORARIOS_URL
  );
}
...
export async function getAvailableSlotsRange(startDate, endDate, serviceDuration = SERVICE_DURATION) {
  ...
  if (fallbackBaseUrl && useHostedBackend) {
...
export async function bookAppointment(bookingData) {
  ...
  const response = await fetch(useHostedBackend ? fallbackBaseUrl : '/api/book', {
    method: 'POST',

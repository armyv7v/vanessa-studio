# Design System — Vanessa Nails Studio
## Based on Apple principles · Brand palette from @nailsvanessa.cl

> **This section (§0) OVERRIDES the color palette in the Apple reference below.**
> All other Apple principles (typography scale, whitespace, cinematic rhythm, glass nav, shadow system) apply as documented.

---

## 0. Vanessa Nails Brand Identity

### Brand Palette (extracted from @nailsvanessa.cl Instagram)

| Token | Hex | Role |
|-------|-----|------|
| `--brand` | `#E11B74` | **Primary CTA, accent, focus ring** — vibrant magenta fuchsia. The ONLY chromatic brand color (Apple Blue equivalent). |
| `--brand-light` | `#F04A94` | Hover/lighter state of primary |
| `--brand-lighter` | `#F87CB3` | Soft glow orbs, tertiary accents |
| `--brand-lightest` | `#FDE8F2` | Section kicker backgrounds |
| `--brand-dark` | `#B8105D` | Active/pressed states |
| `--gold` | `#C5A059` | **Metallic gold** — second accent, used for borders, icons, premium markers |
| `--gold-light` | `#D4B878` | Gold highlights, hover borders |
| `--gold-lightest` | `#FBF4E3` | Gold chip backgrounds |
| `--bg-marble` | `#F9F9F9` | **Primary "light section"** background (Apple `#f5f5f7` equivalent) |
| `--bg-blush` | `#FEF0F4` | Warm blush tint for glass panels |
| `--ink` | `#1A1A1A` | Body text, near-black headings |
| `--ink-medium` | `#3D1929` | Primary text on light backgrounds |
| `--ink-muted` | `#7A3A55` | Secondary text, descriptions |
| `--ink-faint` | `#B0708A` | Placeholders, tertiary text |

### Aesthetic Signature
- **Vibrant Fuchsia + Metallic Gold** on marble/cream backgrounds — the brand identity is luxury beauty, not soft pastel
- Photography aesthetic: warm studio white, cream/marble surfaces, metallic nail art, high saturation nail colors
- The fuchsia (`#E11B74`) is MORE saturated and DARKER than the previous `#ff5fa2` palette — this is intentional
- Gold replaces the previous amber/yellow — it reads as premium, not decorative

### Do's (brand-specific)
- Use `--brand` (`#E11B74`) for all interactive elements, exactly as Apple uses `#0071e3`
- Use `--gold` (`#C5A059`) for secondary accents, borders on marble backgrounds
- Keep backgrounds in the marble/blush/cream range — not white, not pink
- Flow guide step numbers use the **gold gradient** (not brand magenta) — they are milestones, not CTAs

### Don'ts (brand-specific)
- Don't use the old `#ff5fa2` / `#ff5ea0` pink — it reads as generic "girly pink", not brand
- Don't mix gold and brand magenta in the same element — that would clash; keep them separated by role
- Don't use gradients as primary backgrounds — the page-light gradient is SUBTLE, not the main event

---

# Apple Design System Reference (Architecture & Typography)

> **Below is the original Apple-inspired getdesign template.** Use it for typography scale, spacing, shadow system, responsive breakpoints, and component architecture. The COLOR PALETTE section below is overridden by §0 above.
